class PopulateReadersFromCheckouts < ActiveRecord::Migration
  def change
    #grab all the checkout records for each record, create a record
    #in the readers table with that checkout's reader_name
    #populate the reader_id colun on my checkout record
    #with that reader record's ID
